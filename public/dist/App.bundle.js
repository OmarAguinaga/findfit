...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/gyms/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // when someone clicks on the marker, show the detaikls of that place
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        var html = '\n          <div class="popup">\n            <a href="/gym/' + this.place.slug + '">\n              <img src="/uploads/' + (this.place.photo || 'gym.jpg') + '" alt="' + this.place.name + '" />\n              <p>' + this.place.name + ' - ' + this.place.location.address + '</p>\n            </a>\n          </div>\n        ';
        infoWindow.setContent(html);
        infoWindow.open(map, this);
      });
    });

    // the zoom the map to fit all the markers
